...
const EditHiveModal = ({ hive, isOpen, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (hive) {
      ...
      try {
        const saved = JSON.parse(localStorage.getItem(`hexora_hive_${hive.id}`) || '{}');
        setFormData({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.warn('Backend hive update failed:', err.message);
    }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md shadow-2xl">
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">{t.editHive.location}</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">{t.editHive.note}</label>
            <textarea
              rows={3}
              value={formData.note}
